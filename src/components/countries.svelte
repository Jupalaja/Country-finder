		...
		countriesList = [
			...originalData.filter((country) => {
				if (searchValue.region) {
					return (
						country.region === searchValue.region &&
						country.nativeName.toLowerCase().includes(searchValue.search)
					);
				}
...
<section
	class="bg-purple-600 container h-20 px-5 mx-auto xl:px-0
...
>
	{#each countriesList as country}
		<div class="rounder-md shadow-md">
			<div
				class="image bg-no-repeat bg-cover bg-center"
