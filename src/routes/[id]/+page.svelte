...
{#if selectedCountry}
	<section class="container px-5 xl:px-0 mx-auto max-ws-screen-xl">
		<button on:click={() => goto('/')} class="shadow h-8 w-32 rounded dark:dark-light mb-10">
			<i class="mr-3 inline-block fas fa-long-arrow-alt-left" />
			<span>Back</span>
		</button>
					...
					<div class="column">
						<p class="mb-3">
							...
							{#each selectedCountry.tld as tld, i}
								<span
									>{tld}{#if i !== selectedCountry.tld.length - 1}, {/if}</span
								>
							{/each}
						</p>
						<p class="mb-3">
							...
							{#each Object.keys(selectedCountry.languages) as key, i}
								<span
									>{selectedCountry.languages[
										key
									]}{#if i !== Object.keys(selectedCountry.languages).length - 1}, {/if}
								</span>
							{/each}
						</p>
					</div>
